...
model is in [`planrace/evidence.py`](planrace/evidence.py). The current backward-
compatible schema is [`schemas/evidence-manifest-v2.schema.json`](schemas/evidence-manifest-v2.schema.json);
the frozen v1 schema remains available for historical manifests.

## Signature format

Schema versions `planrace/evidence/1` and `planrace/evidence/2` use the following
...
5. Prefix the bytes with the matching `planrace-evidence-manifest/v1` or `/v2`
   domain and one null byte.
...
signature verification fails. Without `--expected-signer`, it deliberately says
`SIGNATURE VALID; CLAIMS UNVERIFIED`: signature integrity alone does not prove
chain inclusion or truth. Supply a separately trusted validator hotkey when one
exists. `summarize` performs the same integrity verification before emitting
headline metrics.

Protocol-v2 score entries use explicit `strategy_digest` and `schedule_digest`
fields. The historical SQL `result_hash` and `reference_hash` fields must be
null in evidence/2 and are never overloaded with unrelated digests.
